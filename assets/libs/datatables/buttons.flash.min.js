(function (g) { typeof define === 'function' && define.amd ? define(['jquery', 'datatables.net', 'datatables.net-buttons'], function (k) { return g(k, window, document) }) : typeof exports === 'object' ? module.exports = function (k, l) { k || (k = window); if (!l || !l.fn.dataTable)l = require('datatables.net')(k, l).$; l.fn.dataTable.Buttons || require('datatables.net-buttons')(k, l); return g(l, k, k.document) } : g(jQuery, window, document) })(function (g, k, l, q) {
  function w (a) {
    for (var b = ''; a >= 0;) {
      b = String.fromCharCode(a % 26 + 65) + b, a = Math.floor(a / 26) -
1
    } return b
  } function o (a, b, d) { var c = a.createElement(b); d && (d.attr && g(c).attr(d.attr), d.children && g.each(d.children, function (a, b) { c.appendChild(b) }), d.text !== null && d.text !== q && c.appendChild(a.createTextNode(d.text))); return c } function C (a, b) {
    var d = a.header[b].length, c; a.footer && a.footer[b].length > d && (d = a.footer[b].length); for (var e = 0, f = a.body.length; e < f; e++) {
      if (c = a.body[e][b], c = c !== null && c !== q ? c.toString() : '', c.indexOf('\n') !== -1 ? (c = c.split('\n'), c.sort(function (a, b) { return b.length - a.length }), c = c[0].length)
        : c = c.length, c > d && (d = c), d > 40) return 52
    } d *= 1.3; return d > 6 ? d : 6
  } function x (a) {
    r === q && (r = v.serializeToString(g.parseXML(n['xl/worksheets/sheet1.xml'])).indexOf('xmlns:r') === -1); g.each(a, function (b, d) {
      if (g.isPlainObject(d))x(d); else {
        if (r) {
          var c = d.childNodes[0], e, f, h = []; for (e = c.attributes.length - 1; e >= 0; e--) { f = c.attributes[e].nodeName; var i = c.attributes[e].nodeValue; f.indexOf(':') !== -1 && (h.push({name: f, value: i}), c.removeAttribute(f)) }e = 0; for (f = h.length; e < f; e++) {
            i = d.createAttribute(h[e].name.replace(':', '_dt_b_namespace_token_')),
            i.value = h[e].value, c.setAttributeNode(i)
          }
        }c = v.serializeToString(d); r && (c.indexOf('<?xml') === -1 && (c = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + c), c = c.replace(/_dt_b_namespace_token_/g, ':')); c = c.replace(/<([^<>]*?) xmlns=""([^<>]*?)>/g, '<$1 $2>'); a[b] = c
      }
    })
  } var j = g.fn.dataTable, h = {version: '1.0.4-TableTools2',
    clients: {},
    moviePath: '',
    nextId: 1,
    $: function (a) {
      typeof a === 'string' && (a = l.getElementById(a)); a.addClass || (a.hide = function () { this.style.display = 'none' }, a.show = function () {
        this.style.display =
''
...
c + '"/><param name="wmode" value="transparent"/></object>')
    ...
    destroy: function () { var a = this; this.domElement && this.div && (g(this.div).remove(), this.div = this.domElement = null, g.each(h.clients, function (b, d) { d === a && delete h.clients[b] })) },
    reposition: function (a) { a && ((this.domElement = h.$(a)) || this.hide()); if (this.domElement && this.div) { var a = h.getDOMObjectPosition(this.domElement), b = this.div.style; b.left = '' + a.left + 'px'; b.top = '' + a.top + 'px' } },
...
this.cssEffects && this.domElement.addClass('active'); break; case 'mouseup':this.domElement && this.cssEffects && (this.domElement.removeClass('active'), this.recoverActive = !1)
      } if (this.handlers[a]) for (var c = 0, e = this.handlers[a].length; c < e; c++) { var f = this.handlers[a][c]; if (typeof f === 'function')f(this, b); else if (typeof f === 'object' && f.length == 2)f[0][f[1]](this, b); else if (typeof f === 'string')k[f](this, b) }
    }}; h.hasFlash = function () {
...
navigator.mimeTypes['application/x-shockwave-flash'] !== q && navigator.mimeTypes['application/x-shockwave-flash'].enabledPlugin) return !0
    ...
    z = function (a) { return a.newline ? a.newline : navigator.userAgent.match(/Windows/) ? '\r\n' : '\n' }, A = function (a, b) {
      for (var d = z(b), c = a.buttons.exportData(b.exportOptions), e = b.fieldBoundary, f = b.fieldSeparator, g = RegExp(e, 'g'), h = b.escapeChar !== q ? b.escapeChar : '\\', k = function (a) { for (var b = '', c = 0, d = a.length; c < d; c++)c > 0 && (b += f), b += e ? e + ('' + a[c]).replace(g, h + e) + e : a[c]; return b }, l = b.header ? k(c.header) + d : '', o = b.footer && c.footer ? d + k(c.footer) : '', m = [], p = 0, j = c.body.length; p < j; p++)m.push(k(c.body[p])); return {str: l + m.join(d) +
o,
      rows: m.length}
    }, u = {available: function () { return h.hasFlash() }, init: function (a, b, d) { h.moviePath = j.Buttons.swfPath; var c = new h.Client(); c.setHandCursor(!0); c.addEventListener('mouseDown', function () { d._fromFlash = !0; a.button(b[0]).trigger(); d._fromFlash = !1 }); y(c, b); d._flash = c }, destroy: function (a, b, d) { d._flash.destroy() }, fieldSeparator: ',', fieldBoundary: '"', exportOptions: {}, title: '*', messageTop: '*', messageBottom: '*', filename: '*', extension: '.csv', header: !0, footer: !1}, v = '', v = typeof k.XMLSerializer === 'undefined'
      ? new function () { this.serializeToString = function (a) { return a.xml } }() : new XMLSerializer(), r, n = {'_rels/.rels': '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>',
...
j.Buttons.version + '/swf/flashExport.swf'; j.Api.register('buttons.resize()', function () { g.each(h.clients, function (a, b) { b.domElement !== q && b.domElement.parentNode && b.positionElement() }) }); j.ext.buttons.copyFlash = g.extend({}, u, {className: 'buttons-copy buttons-flash',
...
f + f + e); d.messageBottom && (e = e + f + f + d.messageBottom); c.customize && (e = c.customize(e, c, b)); a.setAction('copy'); t(a, e); this.processing(!1); b.buttons.info(b.i18n('buttons.copyTitle', 'Copy to clipboard'), b.i18n('buttons.copySuccess', {_: 'Copied %d rows to clipboard', 1: 'Copied 1 row to clipboard'}, data.rows), 3E3)
    ...
    fieldSeparator: '\t',
    fieldBoundary: ''}); j.ext.buttons.csvFlash = g.extend({}, u, {className: 'buttons-csv buttons-flash',
...
c._flash, e = A(b, c), d = b.buttons.exportInfo(c), b = c.customize ? c.customize(e.str, c, b) : e.str; a.setAction('csv'); a.setFileName(d.filename); t(a, b)
    ...
    escapeChar: '"'}); j.ext.buttons.excelFlash = g.extend({}, u, {className: 'buttons-excel buttons-flash',
    text: function (a) { return a.i18n('buttons.excel', 'Excel') },
    action: function (a, b, d, c) {
      this.processing(!0); var a = c._flash, e = 0, f = g.parseXML(n['xl/worksheets/sheet1.xml']), h = f.getElementsByTagName('sheetData')[0], d = {_rels: {'.rels': g.parseXML(n['_rels/.rels'])},
          xl: {_rels: {'workbook.xml.rels': g.parseXML(n['xl/_rels/workbook.xml.rels'])},
            'workbook.xml': g.parseXML(n['xl/workbook.xml']),
            'styles.xml': g.parseXML(n['xl/styles.xml']),
            worksheets: {'sheet1.xml': f}},
          '[Content_Types].xml': g.parseXML(n['[Content_Types].xml'])}, i = b.buttons.exportData(c.exportOptions), k, l, j = function (a) {
          k = e + 1; l = o(f, 'row', {attr: {r: k}}); for (var b = 0, d = a.length; b < d; b++) {
            var j = w(b) + '' + k, i = null; if (a[b] === null || a[b] === q || a[b] === '') if (!0 === c.createEmptyCells)a[b] = ''; else continue; a[b] = g.trim(a[b]); for (var m = 0, p = B.length; m < p; m++) {
...
a[b].match(n.match)) { i = a[b].replace(/[^\d\.\-]/g, ''); n.fmt && (i = n.fmt(i)); i = o(f, 'c', {attr: {r: j, s: n.style}, children: [o(f, 'v', {text: i})]}); break }
            }i || (typeof a[b] === 'number' || a[b].match && a[b].match(/^-?\d+(\.\d+)?$/) && !a[b].match(/^0\d+/) ? i = o(f, 'c', {attr: {t: 'n', r: j}, children: [o(f, 'v', {text: a[b]})]}) : (n = !a[b].replace ? a[b] : a[b].replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, ''), i = o(f, 'c', {attr: {t: 'inlineStr', r: j}, children: {row: o(f, 'is', {children: {row: o(f, 't', {text: n})}})}}))); l.appendChild(i)
          }h.appendChild(l)
          e++
...
r; s++)j(i.body[s], e); c.footer && i.footer && (j(i.footer, e), g('row:last c', f).attr('s', '2')); p.messageBottom && (j([p.messageBottom], e), m(e, i.header.length - 1)); j = o(f, 'cols'); g('worksheet', f).prepend(j); m = 0; for (s = i.header.length; m < s; m++)j.appendChild(o(f, 'col', {attr: {min: m + 1, max: m + 1, width: C(i, m), customWidth: 1}})); c.customize && c.customize(d, c, b); x(d); a.setAction('excel'); a.setFileName(p.filename); a.setSheetData(d); t(a, ''); this.processing(!1)
    ...
    extension: '.xlsx',
    createEmptyCells: !1}); j.ext.buttons.pdfFlash =
g.extend({}, u, {className: 'buttons-pdf buttons-flash',
  text: function (a) { return a.i18n('buttons.pdf', 'PDF') },
  action: function (a, b, d, c) {
    this.processing(!0); var a = c._flash, d = b.buttons.exportData(c.exportOptions), e = b.buttons.exportInfo(c), f = b.table().node().offsetWidth, g = b.columns(c.columns).indexes().map(function (a) { return b.column(a).header().offsetWidth / f }); a.setAction('pdf'); a.setFileName(e.filename); t(a, JSON.stringify({title: e.title || '',
